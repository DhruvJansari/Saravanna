...
// Couple's images - perfectly matched for split screen effect (man on left, woman on right)
const COUPLE_IMAGES = {
  manImage: "./menss.png",
  womanImage: "./womenss.png",
};

// Alternative couple images (fallback)
...
const SplitScreenCouple = () => {
  ...
  const [imagesLoaded, setImagesLoaded] = useState({ man: false, woman: false });
  ...
  // Handle mouse/touch movement for the slider
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8">
        ...
        >
          ...
          {(!imagesLoaded.man || !imagesLoaded.woman) && (
            <div className="absolute inset-0 bg-gradient-to-r from-stone-800 to-stone-700 flex items-center justify-center z-30">
              <div className="text-center">
                ...
                <p className="text-white font-['Inter']">Loading couple's portrait...</p>
              </div>
            </div>
          )}
          ...
          >
            ...
            <div className="absolute bottom-6 left-6 md:bottom-8 md:left-8 text-white z-10 pointer-events-none transition-opacity duration-300" style={{ opacity: sliderPosition < 30 ? 1 : sliderPosition > 70 ? 0.3 : 0.7 }}>
              <p className={`text-xs md:text-sm tracking-wider uppercase ${FONT_STYLE.body} font-semibold bg-black/30 backdrop-blur-sm px-3 py-1 rounded-full inline-block`}>
                His Story
              </p>
              <h3 className={`text-xl md:text-3xl ${FONT_STYLE.heading} font-bold mt-2 drop-shadow-lg`}>
                Strength & Grace
              </h3>
            </div>
          </div>
          ...
          >
            ...
            <div className="absolute bottom-6 right-6 md:bottom-8 md:right-8 text-white z-10 pointer-events-none transition-opacity duration-300 text-right" style={{ opacity: sliderPosition > 70 ? 1 : sliderPosition < 30 ? 0.3 : 0.7 }}>
              <p className={`text-xs md:text-sm tracking-wider uppercase ${FONT_STYLE.body} font-semibold bg-black/30 backdrop-blur-sm px-3 py-1 rounded-full inline-block`}>
                Her Elegance
              </p>
              <h3 className={`text-xl md:text-3xl ${FONT_STYLE.heading} font-bold mt-2 drop-shadow-lg`}>
